...
interface CourseFiltersProps {
  onFilterChange: (filters: { city: string; section: string; search: string }) => void
}
...
export default function CourseFilters({ onFilterChange }: CourseFiltersProps) {
  ...
  const t = useMemo(() => {
    const translations = {
      ar: {
        ...
        sectionLabel: "القسم",
        searchLabel: "البحث",
        cities: ["جميع المدن", "لندن", "دبي", "اسطنبول", "باريس", "روما", "برشلونة", "مدريد", "البندقية"],
        sections: [
          "جميع الأقسام",
          "التواصل المؤسسي",
          "المراسم والاتكيت",
      ...
      en: {
        ...
        searchLabel: "Search",
        cities: ["All Cities", "London", "Dubai", "Istanbul", "Paris", "Rome", "Barcelona", "Madrid", "Venice"],
        sections: [
          "All Sections",
  ...
  useEffect(() => {
    ...
    if (sectionParam && t.sections.includes(sectionParam)) initialSection = sectionParam
    ...
    onFilterChange({ city: initialCity, section: initialSection, search: initialSearch })
  ...
  return (
    <Card
      ...
      className="mb-8 bg-gradient-to-r from-royal-50 to-crimson-50 dark:from-gray-800 dark:to-gray-900 border border-royal-200 dark:border-gray-700 shadow-lg"
    >
      <CardContent className="p-6">
        ...
        <div className="grid md:grid-cols-4 gap-4">
          ...
          <div className="relative">
            <Search
              className={`absolute ${isArabic ? "right-3" : "left-3"} top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400 dark:text-gray-500`}
            />
            <Input
              placeholder={t.searchPlaceholder}
              value={searchTerm}
              onChange={(e) => handleSearchChange(e.target.value)}
              className={`${isArabic ? "pr-10 text-right" : "pl-10 text-left"} bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:placeholder:text-gray-400`}
            />
          </div>
          ...
          <Button
            ...
            className={`flex items-center gap-2 bg-white hover:bg-gray-50 dark:bg-gray-700 dark:hover:bg-gray-600 dark:border-gray-600 dark:text-white ${isArabic ? "flex-row-reverse" : ""}`}
          >
            <X className="w-4 h-4" />
            {t.clearFilters}
          </Button>
        ...
        {/* Active Filters */}
        {(selectedCity !== t.cities[0] || selectedSection !== t.sections[0] || searchTerm) && (
          <div
            className={`mt-4 flex flex-wrap gap-2 ${isArabic ? "justify-end text-right" : "justify-start text-left"}`}
          >
            <span className="text-sm text-gray-600 dark:text-gray-400">{t.activeFilters}</span>
            {selectedCity !== t.cities[0] && (
              <span className="px-3 py-1 bg-royal-100 text-royal-700 dark:bg-royal-900 dark:text-royal-300 rounded-full text-sm">
