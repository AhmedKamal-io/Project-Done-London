...
export default async function EventPageWrapper({ params }: any) {
  ...
  const cookieStore = cookies();
  const currentLang = (cookieStore.get('language')?.value || 'en') as 'ar' | 'en';

  // 4) إنشاء Course Schema
  const courseSchema = {
    "@context": "https://schema.org",
    "@type": "Course",
    "name": matchedCourse.translations?.[currentLang]?.name || matchedCourse.translations?.en?.name,
    "description": matchedCourse.translations?.[currentLang]?.description || matchedCourse.translations?.en?.description,
    "provider": {
      "@type": "Organization",
    ...
    "hasCourseInstance": {
      ...
      "location": {
        "@type": "Place",
        "name": matchedCourse.translations?.[currentLang]?.city || matchedCourse.translations?.en?.city,
        "address": {
          "@type": "PostalAddress",
          "addressLocality": matchedCourse.translations?.[currentLang]?.city || matchedCourse.translations?.en?.city
        }
      },
    ...
    "offers": {
      "@type": "Offer",
      "category": matchedCourse.translations?.[currentLang]?.section || matchedCourse.translations?.en?.section,
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 3,
        "name": matchedCourse.translations?.[currentLang]?.name || matchedCourse.translations?.en?.name,
        "item": `https://www.lampr.ac/event/${slugFromURL}`
      }
    ]
  };
  ...
  const faqSchema = {
    ...
    "mainEntity": [
      ...
      },
      {
        ...
        "acceptedAnswer": {
          "@type": "Answer",
          "text": `${currentLang === 'ar' ? 'تُعقد الدورة في' : 'The course is held in'} ${matchedCourse.translations?.[currentLang]?.city || matchedCourse.translations?.en?.city}.`
        }
      },
      {
        "@type": "Question",
